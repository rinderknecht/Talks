...
\begin{slide}
  ...
  \begin{itemize}
    ...
    \item Yann Régis-Gianas wrote the front-end of Menhir, and
      ...
      to the static typing of the automata's stack: \url{https://cambium.inria.fr/~fpottier/publis/fpottier-regis-gianas-typed-lr.pdf}
  \end{itemize}
\end{slide}
...
\begin{slide}
  ...
  \begin{itemize}
    ...
    \item Despite the subset of TypeScript needed for writing JsLIGO
      contracts is comfortable enough (some computations can be
      expressed in several ways, for instance), the exact frontier
      with \TypeScript{} is not defined explicitly: a syntax error
      cannot tell whether the input is actually valid \TypeScript{} or
      not.

    \item Note how, in the previous example, the parser cannot
...
\TypeScript{}. If the code compiles, but the semantics differs (for
example, due to different implicit type coercions), the LIGO team
would consider it a bug, but it could be silent (we never found an
example, though).
...
\begin{slide}
  ...
  \begin{itemize}
    ...
    \item have the LIGO compiler
      inform the developers of the differences between \TypeScript{}
      and \JsLIGO{},

    \item and avoid writing a lexer and parser in \textsf{ocamllex}
      and \Menhir{} for \TypeScript{}, a new approach was proposed,
      namely use a third\hyp{}party \TypeScript{} parser.
    ...
    \item It is difficult to assess the correctness, let alone the
      conformance of that parser. First, contrary to \Menhir{} (which
      has a \textsf{Coq} back\hyp{}end!), we do not have a way to
      prove that the parser indeed recognises the language generated
      by the grammar. Second, there is no more formal grammar of
      \TypeScript{} published by Microsoft since version 1.8 (current
      is 5.8.3) ---~in 2016.
