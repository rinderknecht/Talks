...
\begin{frame}
  ...
  \begin{itemize}
    ...
    \item I went on to work as an engineer, a researcher and a
      ...
      Korea, Hungary, Sweden). I also worked in private companies.
...
A set constraint has the form $E_1 \subseteq E_2$, where $E_1$ and
$E_2$ are \emph{set expressions}. Examples are \textsf{0} (the empty
set), \textsf{1} (the set of all terms), $\alpha$ (a set-valued
variable), $c(E_1, E_2)$ (a constructor application), and the union,
intersection or complement of set expressions. Given a set of
constructors ${\cal C}$, where each $c \in \mathcal{C}$ has arity
$a(c)$, set expressions are defined by the grammar:
\begin{equation*}
E ::= \textsf{0} \mid \textsf{1} \mid c (E_1, \ldots,
...
programs. For us, solving these constraints means constructing the
smallest supersets of the sets of the computed values (least
fixpoint).
...
(that is, $v \subseteq \textsf{Int} \, \cup \, \textsf{Float}$) and
from the other use that $v$ cannot be an integer (namely, $v \subseteq
\neg\textsf{Int}$). Combining these constraints, we can infer that $v$
must be a floating point number (to wit, $v \subseteq (\textsf{Int} \,
...
\begin{frame}
  ...
  \begin{itemize}
    ...
    \item The economic protocol of Tezos handles a ledger of
      ...
      replicated database, immutable entries and resilience to
      malicious users.
