...
\begin{frame}
\frametitle{Illustrated professional overview}
...
\begin{itemize}
  ...
  and soundness proof of an encoding rule). \textbf{Theoretical
...
\begin{itemize}
  ...
  \item At \emph{Léonard de Vinci}, XML-based web-site frameworks
  ...
  \textbf{``language'' aspect of the problems}.
...
%
\begin{frame}
\frametitle{Projects and collaborations}
...
\begin{itemize}
  ...
  \item At \emph{Léonard de Vinci}, I was a member of the European
...
\begin{itemize}
  ...
  \item During my Ph.D. (1993-1996), I teached at univ. René Diderot
  ...
  \item Teaching assistant at \emph{Institut Supérieur d'Électronique
...
\begin{itemize}
  ...
  \item At \emph{Léonard de Vinci}, I taught software engineering,
...
%
\begin{frame}
\frametitle{Visiting Professor at Information and Communications University (2001-2002)}
...
%
\begin{frame}
\frametitle{Assistant Professor at \emph{Léonard de Vinci} (2003-2004)}
...
\begin{itemize}
  ...
  \begin{itemize}
    ...
    \item a question-answering mining web-platform,

    \item the formal design review of \ASN and BER.
...
\begin{itemize}
  ...
  \item \textbf{we want to show that \ASN and the BER are, themselves,
  well designed.}
...
% Modeling
\begin{frame}
\frametitle{Modeling (assumptions and encoding)}
...
\begin{itemize}
  ...
  the sound design of the BER: \textbf{the composition of encoding and
...
to \ASN. Hence \textbf{this proposition is the link between the design
...
\begin{itemize}
  ...
  subset which has the same expressivity, we call \textbf{BER domain}.

  \item In fact it is even possible to reduce further the BER domain
  to a smaller subset, we call \textbf{\core}, in which formal
  reasoning is easier.
...
\textbf{An inference rule can be interpreted also from a computational
...
%
\begin{frame}
\frametitle{Inference rules (continued)}
...
\begin{itemize}
  ...
  \item Let us note \textbf{$\coding{\Gamma}{v}{\T}{c}$} the judgement
  ``In the environment $\Gamma$, the value $v$ is encoded into the
  code $c$, following the type \T{}.''  (The environment models the
  \ASN module.)

  \item Let us note \textbf{$\decoding{\Gamma}{c}{\T}$} the decoding
  of $c$ in the environment $\Gamma$ according to type \T{}.

  \item We already noted \textbf{$v_1 \approx v_2$} the equivalence of
  values $v_1$ and $v_2$

  \item and \textbf{$c_1 \sim c_2$} the equivalence of codes $c_1$ and
  $c_2$.
...
%
\begin{frame}
\frametitle{Formal methods for the customer}

\begin{itemize}

  \item We prove the soundness theorem with a technique called
  \textbf{structural induction} (on the height of the proof trees) and
  \textbf{case analysis}.
...
%
\begin{frame}
\frametitle{Formal methods for the software company}
...
\begin{itemize}
  ...
  close to an \textbf{operational model}, i.e. a real implementation,
  \textbf{based on a functional language}.
